...
def train(dataset):
    ...
    for epoch in range(args.epochs):
        ...
        for i_batch, (image, label) in tqdm(enumerate(train_loader)):
            y = model(image.cuda())
            label = torch.LongTensor(label.view(-1).numpy()).cuda()
...
def test(images):
    ...
    if os.path.exists('model.pth'):
        model.load_state_dict(torch.load('model.pth', map_location='cuda:0'), strict=False)

    y = model(torch.Tensor(images).cuda())
    pred_prob, pred_label = torch.max(y, dim=1)
    print(pred_label)
    df = pd.read_excel('pred_data.xls')
    df['predict_label'] = pred_label.cpu()
